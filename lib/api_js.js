...
/**
 * 多台服务器负载均衡时，ticketToken需要外部存储共享。
 * 需要调用此registerAgenticketHandle来设置获取和保存的自定义方法。
 *
 * Examples:
 * ```
 * api.registerAgenticketHandle(getTicketToken, saveTicketToken);
 * // getTicketToken
 * function getTicketToken(callback) {
 *  settingModel.getItem({key: 'weixin_ticketToken'}, function (err, setting) {
 *    if (err) return callback(err);
 ...
 * @param {Function} saveTicketToken 存储外部ticketToken的函数
 */
exports.registerAgenticketHandle = function (getTicketToken, saveTicketToken) {
  if (!getTicketToken && !saveTicketToken) {
