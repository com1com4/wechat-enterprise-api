...
/**
 * 多台服务器负载均衡时，ticketToken需要外部存储共享。
 * 需要调用此registerAgentTicketHandle来设置获取和保存的自定义方法。
 *
 * Examples:
 * ```
 * api.registerAgentTicketHandle(getTicketToken, saveTicketToken);
 * // getTicketToken
 * function getTicketToken(callback) {
 *  settingModel.getItem({key: 'weixin_ticketToken'}, function (err, setting) {
 *    if (err) return callback(err);
 ...
 * @param {Function} saveTicketToken 存储外部ticketToken的函数
 */
exports.registerAgentTicketHandle = function (getTicketToken, saveTicketToken) {
  if (!getTicketToken && !saveTicketToken) {
    this.agentTicketStore = {};
  }
  this.getAgentTicketToken = getTicketToken || function (callback) {
    callback(null, this.AgentTicketStore);
  };

  this.saveAgentTicketToken = saveTicketToken || function (ticketToken, callback) {
    this.agentTicketStore = ticketToken;
    if (process.env.NODE_ENV === 'production') {
      console.warn("Dont save agent ticket in memory, when cluster or multi-computer!");
    }
    callback(null);
  };
};
 ...
 * @param {Function} callback 回调函数
 */
exports.getAgentTicket = function (callback) {
  this.preRequest(this._getAgentTicket, arguments)
}

exports._getAgentTicket = function (callback) {
  var that = this;
  var url = this.prefix + 'ticket/get?access_token=' + this.token.accessToken + '&type=agent_config';
  this.request(url, {dataType: 'json'}, wrapper(function(err, data) {
    if (err) {
      return callback(err);
    }
 ...
 *  url: 'http://www.xxx.com' 
 * };
 * api.getJsAgentConfig(param, callback);
 * ```
 *
 * Callback:
 * - `err`, 调用失败时得到的异常
 ...
 * @param {Function} callback 回调函数
 */
exports.getJsAgentConfig = function (param, callback) {
  var that = this;
  that.getAgentTicketToken(function (err, ticket) {
    if (err) {
      return callback(err);
    }
    var response = function (ticket) {
    ...
    if (ticket && new Ticket(ticket.ticket, ticket.expireTime).isValid()) {
      ...
      that.getAgentTicket(function (err, ticket) {
        if (err) {
          return callback(err);
        }
        response(ticket);
